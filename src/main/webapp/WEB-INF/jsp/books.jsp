...
<head>
    ...
    <script>document.getElementById("search_query").addEventListener("keydown", function (e) {
        if (!e) {
            var e = window.event;
        }
        ...
        // Enter is pressed
        if (e.keyCode == 13) {
            document.getElementById("search_query").parentNode.submit();
        }
    }, false);</script>
</head>
...
<div class="container">
    <form action="/search" method="post">
        Order by:
        <select class="inputSelect" name="order_and_sort">
            <option value="0">Book name, asc</option>
            <option value="1">Book name, desc</option>
            <option value="2">Publishing year, asc</option>
            <option value="3">Publishing year, desc</option>
        </select>
        Search by:
        <select name="search_type" class="inputSelect">
            <option value="0">Author</option>
            <option value="1">Book</option>
        </select>
        <input type="text" name="search_query">
        <span class="glyphicon glyphicon-search" aria-hidden="true" onclick="parentNode.submit()"></span>
    </form>
            ...
            <tr>
                ...
                <td>
                    <form action="/download" method="post">
                        <input name="bookId" type="text" hidden value="${book.getId()}">
                        <input class="download-submit" value="" type="submit">
                    </form>
                </td>
                <td><fmt:formatDate value="${book.getUploadTimestamp()}"/></td>
            </tr>
            </c:forEach>
            </tbody>
        </table>
